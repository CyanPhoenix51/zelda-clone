...
func _process(delta: float) -> void:
	...
	_rotate_from_vector(joy_dir * delta * Vector2(horizontal_acceleration, vertical_acceleration))

#func _input(event: InputEvent) -> void:
	#if event is InputEventMouseMotion:
		#_rotate_from_vector(event.relative * mouse_acceleration)
	
func _rotate_from_vector(v: Vector2):
